...
| POST | `/v1/admin/vocabularies/quick/bulk` | JWT (admin) | Bulk quick-create phase 2. Body: `{ lemmas: string[] (1–500), language? }`. Creates one enrichment job per lemma under a shared `batchId`, skipping lemmas that already have a pending job or an existing system vocab. Returns 202 `{ batchId, accepted, skipped }` (`batchId` is null when everything was skipped). |
