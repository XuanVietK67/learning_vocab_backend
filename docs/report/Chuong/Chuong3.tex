...
Cơ sở khoa học của động cơ học là hiện tượng \textit{đường cong lãng quên}
(forgetting curve) do Ebbinghaus phát hiện: khả năng ghi nhớ suy giảm theo
hàm mũ theo thời gian nếu không được củng cố \cite{ebbinghaus1913memory},
mô hình hoá đơn giản qua công thức~\eqref{eq:forgetting}:
...
với \( R \) là mức ghi nhớ còn lại sau thời gian \( t \) và \( S \) là
\textit{độ bền trí nhớ} (memory stability). Hiệu ứng giãn cách (spacing
effect) cho thấy ôn tập ngắt quãng cải thiện ghi nhớ dài hạn rõ rệt so với
học dồn \cite{cepeda2006distributed}; đây là nền tảng của \textit{hệ thống
ôn tập ngắt quãng} (Spaced Repetition System --- SRS) mà hệ thống hiện thực
hoá.
...
\textbf{SM-2} từ dự án SuperMemo \cite{wozniak1990optimization}. Mỗi cặp
(người dùng, từ) được mô tả bằng hệ số dễ nhớ \( EF \) (khởi tạo \( 2{,}5 \)),
số lần ôn đúng liên tiếp \( n \) và khoảng cách ôn \( I \) (ngày); mỗi lượt
trả lời được quy về một \textit{điểm chất lượng} \( q \in \{0,1,\dots,5\} \).
Hệ số \( EF \) cập nhật theo công thức~\eqref{eq:sm2-ef} (chặn dưới
\( 1{,}3 \) để khoảng ôn không co lại quá nhanh):
...
Khi trả lời sai (\( q < 3 \)), \( n \) đặt lại về 0; ngược lại \( n \) tăng
và khoảng ôn giãn dần theo \( EF' \). Hệ thống \textbf{mở rộng SM-2 bằng các
bước học} (learning steps): một từ mới phải vượt một chuỗi câu hỏi từ dễ đến
khó trong phiên trước khi được SM-2 lập lịch dài hạn, với \( q \) suy từ độ
chính xác ở bước cuối; nhờ đó trạng thái thẻ chuyển dịch \code{new} \( \to \)
\code{learning} \( \to \) \code{review} \( \to \) \code{mastered} theo biểu
đồ hoạt động ở Chương~\ref{chuong:phan-tich-thiet-ke}.

\subsection{Khung tham chiếu trình độ ngôn ngữ CEFR}
\label{subsec:cefr-theory}

Độ khó từ vựng và trình độ người học được chuẩn hoá theo \textbf{Khung tham
chiếu trình độ ngôn ngữ chung châu Âu} (CEFR) của Hội đồng châu Âu
\cite{coe2001cefr}, gồm sáu bậc từ \( A1 \) đến \( C2 \). Mỗi từ được gắn
một bậc CEFR và mỗi người học khai báo trình độ, nhờ đó hệ thống chọn từ,
gợi ý bộ thẻ phù hợp năng lực và để mô hình ngôn ngữ chấm bậc CEFR cho câu
luyện tập.

\subsection{Kiến trúc REST và dịch vụ web}
\label{subsec:rest-theory}

API của hệ thống tuân theo phong cách kiến trúc \textbf{REST}
(Representational State Transfer) do Fielding đề xuất \cite{fielding2000rest}:
mô hình \textit{client--server} tách bạch; tính \textit{phi trạng thái}
(stateless) --- mỗi yêu cầu tự mang token, máy chủ không lưu trạng thái
phiên; định danh tài nguyên bằng \textit{URI} thao tác qua các phương thức
HTTP chuẩn (GET, POST, PATCH, DELETE); và trao đổi dữ liệu \textbf{JSON}.
Tính phi trạng thái là điều kiện mở rộng tầng API theo chiều ngang (NFR-03,
Bảng~\ref{tab:nfr}); API được \textit{phiên bản hoá theo URI} (tiền tố
\code{/v1}) để tiến hoá hợp đồng mà không phá vỡ máy khách cũ.

\subsection{Ánh xạ đối tượng--quan hệ (ORM)}
\label{subsec:orm-theory}

Hệ thống dùng kỹ thuật \textbf{ánh xạ đối tượng--quan hệ} (Object--Relational
Mapping --- ORM) để thu hẹp khoảng cách giữa mô hình hướng đối tượng trong
mã nguồn và mô hình quan hệ trong cơ sở dữ liệu: mỗi bảng là một lớp
\textit{thực thể} (entity), mỗi dòng một đối tượng, khoá ngoại là liên kết
giữa các thực thể; lược đồ tiến hoá qua cơ chế \textit{migration} phiên bản
hoá được. Đánh đổi của ORM là một lớp trừu tượng phụ và nguy cơ truy vấn kém
hiệu quả nếu lạm dụng, được khắc phục bằng đánh chỉ mục truy vấn nóng và truy
vấn tường minh khi cần.
...
tập, hội thoại luyện nói) dựa trên \textbf{mô hình ngôn ngữ lớn} (Large
Language Model --- LLM) --- mạng nơ-ron kiến trúc \textit{Transformer}
\cite{vaswani2017attention} huấn luyện trên khối văn bản lớn để dự đoán token
kế tiếp, nhờ đó sinh, dịch và đánh giá ngôn ngữ theo chỉ dẫn (prompt). Hệ
thống dùng hai họ mô hình mở: \textbf{Gemma} của Google DeepMind
\cite{gemma2025technical} cho chấm điểm câu và dự phòng làm giàu dữ liệu, và
\textbf{Llama~3} của Meta \cite{grattafiori2024llama3} cho hội thoại luyện
nói. LLM được giao vai trò có kiểm soát theo định dạng/\textit{rubric} yêu
cầu; nội dung AI sinh cho kho hệ thống luôn phải qua bước \textit{duyệt} của
quản trị viên trước khi xuất bản, nhằm kiểm soát chất lượng và hạn chế lỗi
"ảo giác" (hallucination).

\subsection{Học ngôn ngữ giao tiếp và luyện nói nhập vai}
\label{subsec:clt-theory}

Phòng luyện nói đặt nền trên \textbf{đường hướng dạy ngôn ngữ giao tiếp}
(Communicative Language Teaching --- CLT) và \textbf{dạy ngôn ngữ theo nhiệm
vụ} (Task-Based Language Teaching --- TBLT)
\cite{richards2014approaches, ellis2003task}: năng lực ngôn ngữ hình thành
tốt nhất qua việc \textit{sử dụng} ngôn ngữ để hoàn thành một nhiệm vụ giao
tiếp có mục đích, thay vì ghi nhớ rời rạc; \textit{nhập vai} (role-play) là
kỹ thuật tiêu biểu. Hạn chế cố hữu của nhập vai trong lớp truyền thống ---
cần một bạn luyện tập (interlocutor) luôn sẵn sàng --- được mô hình ngôn ngữ
lớn khắc phục: nó đóng vai đối thoại mọi lúc, điều chỉnh độ khó theo bậc CEFR
của người học (mục~\ref{subsec:cefr-theory}) và phản hồi chỉnh lỗi mang tính
xây dựng. Đây là cơ sở thiết kế: mỗi kịch bản là một nhiệm vụ giao tiếp, hệ
thống dẫn dắt qua nhiều lượt nói và tổng kết bằng một báo cáo phản hồi định
hướng luyện tập tiếp.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.0cm}|p{5.0cm}|p{6.4cm}|}
    ...
    \hline
    Trí tuệ nhân tạo & Gemma~3 (dự phòng), Groq (Llama~3.3), OPUS-MT, Edge TTS, vi dịch vụ chấm phát âm & Làm giàu/dịch dữ liệu từ vựng, hội thoại luyện nói, sinh âm thanh, chấm điểm luyện tập. \\
    \hline
...
% cloud "DỊCH VỤ AI / NGOÀI\nGemma 3, Groq (Llama 3.3), Edge TTS,\nCloudinary, DV chấm phát âm, OAuth, SMTP" as EXT
...
Hệ thống còn dùng \textbf{Redis} làm kho khoá--giá trị trong bộ nhớ cho hàng
đợi BullMQ \cite{redis2024}, và truy cập cơ sở dữ liệu qua \textbf{TypeORM}
\cite{typeorm2024} (mục~\ref{sec:cong-nghe-backend}); lược đồ được quản lý
bằng các \textit{migration} chạy qua công cụ dòng lệnh của TypeORM, giúp tiến
hoá có kiểm soát và tái lập giữa các môi trường.

% =====================================================================
\section{Công nghệ Frontend}
\label{sec:cong-nghe-frontend}

Ứng dụng khách (frontend) là một dự án độc lập tiêu thụ API REST của backend,
xây dựng bằng \textbf{Next.js 16} theo mô hình \textit{App Router} (hỗ trợ
Server-Side Rendering, Server Components, Server Actions) \cite{nextjs2024}
trên nền \textbf{React~19} \cite{react2024} và \textbf{TypeScript} chế độ
\textit{strict}; dùng chung TypeScript ở hai phía giúp thống nhất kiểu của
hợp đồng API. Giao diện tạo kiểu bằng \textbf{Tailwind CSS v4} (utility-first)
\cite{tailwind2024} với bộ thành phần \textbf{shadcn/ui} trên các primitive
khả truy cập của \textbf{Base UI}; biểu mẫu được kiểm tra bằng \code{zod}
\cite{zod2024}, dùng lại giữa máy khách và Server Action. Đáng chú ý, các
biểu đồ tiến độ (chuỗi ngày học, heatmap) được \textit{dựng thủ công} bằng
CSS/SVG thay vì nhúng thư viện biểu đồ, nhằm giữ gói tải (bundle) nhỏ và kiểm
soát bố cục theo hệ thống thiết kế. Bảng~\ref{tab:frontend-libs} liệt kê các
thư viện frontend chính.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{4.2cm}|p{10.4cm}|}
    ...
    \hline
    \code{next-themes}, \code{input-otp} & Bảng màu định nghĩa sẵn cho chế độ tối (theme-ready)~\cite{nextthemes2024} và ô nhập mã OTP (xác minh email). \\
    \hline
    \code{next/font} (Geist, Plus Jakarta Sans, Newsreader) & Nạp và tối ưu phông chữ tự lưu trữ (self-hosted), tránh dịch chuyển bố cục khi tải phông \cite{nextjs2024}. \\
    \hline
    \code{next/image} & Tối ưu hình ảnh (đổi cỡ, định dạng hiện đại, tải lười) cho ảnh minh hoạ nghĩa và thẻ câu hỏi nhìn hình \cite{nextjs2024}. \\
    \hline
    Web Audio API (\code{getUserMedia} + \code{AudioContext}) & Thu âm giọng đọc của người học ngay trên trình duyệt: lấy PCM thô, hạ tần số về 16~kHz đơn kênh và tự đóng gói tệp WAV cho câu hỏi luyện phát âm \cite{w3cwebaudio, w3cmediacapture}. \\
    \hline
  \end{tabular}
...
Quá trình hiện thực hoá tầng giao diện được trình bày ở
mục~\ref{sec:hien-thuc-frontend}, Chương~\ref{chuong:hien-thuc-hoa}; frontend
giao tiếp với backend hoàn toàn qua các API đã đặc tả, không truy cập trực
tiếp cơ sở dữ liệu.
...
Backend được xây dựng bằng \textbf{NestJS} --- framework Node.js cho
TypeScript theo hướng mô-đun, dựa trên \textit{tiêm phụ thuộc} (dependency
injection) và lập trình hướng khía cạnh \cite{nestjs2024}. NestJS phù hợp
với kiến trúc phân tầng ở mục~\ref{sec:kien-truc-tong-quan}: mỗi mô-đun đóng
gói controller, service, provider riêng; các \textit{guard}, \textit{pipe},
\textit{interceptor} xử lý mối quan tâm xuyên suốt (xác thực, kiểm tra đầu
vào, biến đổi phản hồi) một cách khai báo. Bảng~\ref{tab:backend-libs} liệt
kê các thành phần backend chính.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.6cm}|p{4.0cm}|p{6.8cm}|}
    ...
    \hline
    Gemma~3 (Google AI Studio) & Mô hình ngôn ngữ lớn (LLM) & Dự phòng làm giàu dữ liệu từ vựng (bù trường các nguồn xác định không có); chấm điểm câu luyện viết theo rubric kèm bậc CEFR. \\
    \hline
    Groq (Llama~3.3) & Mô hình ngôn ngữ lớn (LLM) hội thoại & Đóng vai đối thoại theo lượt trong phòng luyện nói; sinh nháp kịch bản; sinh báo cáo phản hồi cuối phiên. \\
    \hline
...
\cite{gemma2025technical} cho hai nhóm tác vụ: (i)~\textit{làm giàu dữ liệu
từ điển} và (ii)~\textit{chấm điểm câu luyện tập} --- đánh giá câu người học
đặt theo thang điểm 0--100 kèm bậc CEFR và nhận xét. Riêng ở khâu làm giàu dữ
liệu, Gemma nay chỉ đóng \textbf{vai trò dự phòng}: pipeline ưu tiên các nguồn
xác định (từ điển, kho ngữ liệu, từ điển song ngữ và dịch máy OPUS-MT, bảng
CEFR) và chỉ gọi Gemma để bù những trường không nguồn nào phủ được, sau một cờ
cấu hình có thể tắt hẳn (chi tiết ở mục~\ref{subsec:impl-enrich}). Mọi lời gọi
LLM đều là tác vụ nặng nên được đẩy sang worker xử lý bất đồng bộ và bị giới
hạn theo hạn mức ngày để bảo vệ khoá API dùng chung.

\subsection{Hội thoại luyện nói bằng Groq và Llama~3.3}
\label{subsec:groq}

Phòng luyện nói sử dụng nền tảng suy luận \textbf{Groq}, chạy các mô hình mở
\textbf{Llama~3.3} của Meta \cite{grattafiori2024llama3}. Groq được chọn cho
tác vụ hội thoại vì độ trễ rất thấp --- yếu tố quyết định trải nghiệm của một
cuộc đối thoại theo lượt, nơi người học chờ phản hồi \textit{tức thì}. Vì lý
do này, khác với các tác vụ làm giàu/chấm điểm (đẩy sang worker nền), lời gọi
hội thoại được thực hiện \textit{đồng bộ ngay trong luồng API}.

Hệ thống dùng hai mô hình ở hai mức chi phí khác nhau: một mô hình \textit{nhanh}
cho từng lượt hội thoại (ưu tiên độ trễ), và một mô hình \textit{mạnh hơn} cho
báo cáo cuối phiên (ưu tiên chất lượng đánh giá, chạy một lần khi kết thúc).
Cả hai đều được yêu cầu trả về \textbf{JSON có cấu trúc} (chế độ JSON) để hệ
thống bóc tách phản hồi, gợi ý chỉnh lỗi và các trường đánh giá một cách tin
cậy. Để chịu lỗi hạn mức của nhà cung cấp, hệ thống cấu hình \textit{nhiều
khoá API} và \textit{xoay vòng khoá} khi gặp mã 429/503; đồng thời bảo vệ tài
nguyên dùng chung bằng hạn mức số phiên theo ngày và giới hạn số lượt mỗi
phiên (chi tiết hiện thực ở Chương~\ref{chuong:hien-thuc-hoa}).
...
Mã nguồn được quản lý bằng \textbf{Git}/GitHub theo quy trình nhánh -- pull
request; môi trường phụ thuộc (PostgreSQL, Redis, pgAdmin) được mô tả bằng
\textbf{Docker Compose} \cite{docker2024} để tái lập đồng nhất giữa các máy.
Chất lượng mã được kiểm soát bằng kiểm tra kiểu TypeScript,
\textbf{ESLint}/\textbf{Prettier} và bộ kiểm thử \textbf{Jest}/\textbf{Supertest};
báo cáo được biên soạn bằng \textbf{\LaTeX} trên Overleaf theo mẫu quy định.
...
toán SM-2, khung trình độ CEFR, kiến trúc REST, ánh xạ đối tượng--quan hệ,
mô hình ngôn ngữ lớn và đường hướng dạy ngôn ngữ giao tiếp/nhập vai --- cùng
ngăn xếp công nghệ được lựa chọn cho từng tầng của hệ thống, từ cơ sở dữ liệu,
frontend, backend, cơ chế xác thực và phân quyền, các dịch vụ trí tuệ nhân tạo
(Gemma~3 và Groq/Llama~3.3) đến bộ công cụ phát triển. Những lựa
