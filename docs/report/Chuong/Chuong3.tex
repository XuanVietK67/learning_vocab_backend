...
API REST của backend. Frontend được xây dựng bằng \textbf{Next.js} ---
framework React hỗ trợ kết xuất phía máy chủ (Server-Side Rendering --- SSR)
và sinh trang tĩnh (Static Site Generation --- SSG) \cite{nextjs2024} ---
trên nền thư viện giao diện \textbf{React} \cite{react2024} và ngôn ngữ
\textbf{TypeScript}. Việc dùng chung TypeScript ở cả frontend và backend
giúp thống nhất kiểu dữ liệu của hợp đồng API giữa hai phía. Phần tạo kiểu
giao diện dùng \textbf{Tailwind CSS} --- framework CSS theo hướng tiện ích
(utility-first) \cite{tailwind2024} --- kết hợp bộ thành phần
\textbf{shadcn/ui}. Bảng~\ref{tab:frontend-libs} liệt kê các thư viện
frontend chính.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{4.2cm}|p{10.4cm}|}
    ...
    \hline
    Next.js, React, TypeScript & Khung ứng dụng, dựng giao diện theo thành phần, định tuyến và kết xuất trang. \\
    \hline
    Tailwind CSS, shadcn/ui & Hệ thống tạo kiểu tiện ích và bộ thành phần giao diện tái sử dụng. \\
    \hline
...
Vì trọng tâm của báo cáo là hệ thống phía máy chủ (xem phạm vi đề tài ở
Chương~\ref{chuong:tong-quan}), phần frontend chỉ được trình bày ở mức ngăn
xếp công nghệ; frontend giao tiếp với backend hoàn toàn qua các API đã đặc
tả, không truy cập trực tiếp cơ sở dữ liệu.
