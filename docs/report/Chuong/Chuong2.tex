...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.2cm}|p{2.4cm}|p{9.0cm}|}
    ...
    \hline
    Người học (User) & Chính & Người dùng cuối: học và ôn tập từ vựng, tạo từ vựng và bộ thẻ cá nhân, luyện viết câu, luyện phát âm, luyện nói theo kịch bản hội thoại, theo dõi tiến độ và tham gia bảng xếp hạng. \\
    \hline
    Quản trị viên (Admin) & Chính & Biên tập và quản lý kho từ vựng hệ thống, chủ đề, bộ thẻ mẫu; soạn và xuất bản kịch bản luyện nói; duyệt nội dung do AI sinh ra; quản lý tài khoản người dùng. \\
    \hline
    Dịch vụ AI sinh ngôn ngữ (Gemma~3) & Phụ & Mô hình ngôn ngữ lớn (LLM) trên Google AI Studio: dự phòng làm giàu dữ liệu từ điển (bù các trường nguồn xác định không có) và chấm điểm câu luyện tập. \\
    \hline
    Dịch vụ AI hội thoại (Groq) & Phụ & Nền tảng suy luận LLM tốc độ cao chạy các mô hình Llama~3.3: đóng vai đối thoại trong phiên luyện nói, sinh phản hồi và gợi ý chỉnh lỗi theo lượt, sinh nháp kịch bản và báo cáo cuối phiên. \\
    \hline
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{1.7cm}|p{3.4cm}|p{9.4cm}|}
    ...
    \hline
    FR-16 & Tạo phiên học & Sinh một phiên học theo chế độ (hằng ngày, theo chủ đề, theo bộ thẻ, ôn tập), kèm tuỳ chọn \textit{luyện lại} cho chế độ theo chủ đề/bộ thẻ; mỗi từ được mở rộng thành một chuỗi câu hỏi từ dễ đến khó tuỳ giai đoạn ghi nhớ, rồi cả phiên được sắp xếp thành các vòng theo dạng câu hỏi. \\
    \hline
...
\subsubsection{Phân hệ luyện nói theo kịch bản (Phòng luyện nói)}

\textit{Phòng luyện nói} (Speaking Room) cho phép người học luyện hội thoại
chủ động theo từng \textit{kịch bản} nhập vai do quản trị viên biên soạn. Hệ
thống đóng vai đối thoại bằng LLM, đối đáp theo lượt, sửa lỗi trực tiếp và
tổng kết bằng một báo cáo phản hồi cuối phiên. Bảng~\ref{tab:fr-speaking}
liệt kê các yêu cầu chức năng của phân hệ này.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{1.7cm}|p{3.4cm}|p{9.4cm}|}
    ...
    \textbf{Mã} & \textbf{Chức năng} & \textbf{Mô tả} \\
    \hline
    FR-25 & Duyệt và gợi ý kịch bản & Người học duyệt kho kịch bản đã xuất bản, lọc theo chủ đề và trình độ CEFR; hệ thống ưu tiên gợi ý kịch bản khớp trình độ của người học. \\
    \hline
    FR-26 & Bắt đầu phiên luyện nói & Người học chọn một kịch bản (kèm tuỳ chọn các từ mục tiêu muốn luyện); hệ thống chốt bản sao (snapshot) thông số kịch bản, trình độ và từ mục tiêu, rồi trả về câu mở đầu của AI. \\
    \hline
    FR-27 & Thực hiện lượt hội thoại & Người học gửi câu nói của mình; AI trả lời để duy trì hội thoại, đồng thời trả về danh sách gợi ý chỉnh lỗi và các từ mục tiêu đã dùng trong lượt. \\
    \hline
    FR-28 & Kết thúc phiên và nhận báo cáo & Khi kết thúc phiên, hệ thống sinh báo cáo phản hồi trên toàn bộ hội thoại: tóm tắt, các lỗi tiêu biểu kèm cách nói tốt hơn, từ mục tiêu đã dùng/bỏ lỡ, trình độ ước lượng và gợi ý luyện tiếp. \\
    \hline
    FR-29 & Quản trị kịch bản luyện nói & Quản trị viên tạo, sửa, xuất bản và gỡ kịch bản; có thể sinh nhanh một bản nháp kịch bản bằng AI từ một mô tả ngắn rồi chỉnh sửa trước khi lưu. \\
    \hline
  \end{tabular}
  \caption{Yêu cầu chức năng phân hệ luyện nói theo kịch bản}
  \label{tab:fr-speaking}
\end{table}
...
(viết câu, phát âm và luyện nói theo kịch bản), \textit{Tiến độ \& cộng
đồng}. Quản trị viên kế thừa người học và bổ sung các gói \textit{Quản trị
nội dung} (từ vựng, chủ đề, kịch bản luyện nói, duyệt nội dung AI) và
\textit{Quản trị người dùng}. Các tác nhân phụ (Gemma~3, Groq, dịch vụ chấm
phát âm, nhà cung cấp đăng nhập) tham gia gián tiếp vào một số use case.
...
% actor "Groq (LLM hội thoại)" as GR
...
%   usecase "Luyện nói theo kịch bản" as UC9
%   usecase "Theo dõi tiến độ" as UC10
%   usecase "Bảng xếp hạng" as UC11
%   usecase "Quản trị từ vựng / chủ đề" as UC12
%   usecase "Tạo nhanh từ vựng bằng AI" as UC13
%   usecase "Quản trị kịch bản luyện nói" as UC14
...
Do số lượng use case lớn, phần này chỉ đặc tả chi tiết năm use case tiêu
biểu, đại diện cho các luồng nghiệp vụ cốt lõi: đăng nhập (FR-02), học theo
phiên ôn tập ngắt quãng (FR-16, FR-17), luyện viết câu (FR-21), tạo nhanh
từ vựng bằng AI (FR-23) và luyện nói theo kịch bản (FR-26, FR-27, FR-28).
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.0cm}|p{11.4cm}|}
    ...
    \textbf{Luồng chính} &
      1.~Người học chọn chế độ học (hằng ngày, theo chủ đề, theo bộ thẻ hoặc ôn tập); chế độ theo chủ đề/bộ thẻ có thể bật \textit{luyện lại} để ôn lại ngay các từ đã học. \newline
      2.~Hệ thống chọn các từ phù hợp, tự ghi danh các từ mới (nếu có). \newline
      3.~Mỗi từ được mở rộng thành chuỗi câu hỏi từ dễ đến khó theo giai đoạn ghi nhớ (mỗi câu hỏi được ký HMAC), rồi cả phiên được sắp xếp thành các \textit{vòng theo dạng câu hỏi}: mỗi dạng trải trên tất cả các từ, theo độ khó tăng dần. \newline
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.0cm}|p{11.4cm}|}
    ...
    \textbf{Tác nhân} & Quản trị viên (hoặc Người học cho từ cá nhân); các nguồn làm giàu xác định (từ điển, kho ngữ liệu, dịch máy OPUS-MT); Gemma~3 (dự phòng) \\ \hline
    \textbf{Mô tả} & Từ một lemma, hệ thống tự dựng dữ liệu từ điển và âm thanh cho từ vựng, ưu tiên các nguồn tra cứu xác định. \\ \hline
    ...
    \textbf{Luồng chính} &
      ...
      3.~Worker phân giải mục từ điển rồi điền nghĩa, ví dụ (kho ngữ liệu), bản dịch (từ điển song ngữ/OPUS-MT) và bậc CEFR từ các nguồn xác định, chỉ dùng Gemma~3 dự phòng cho trường còn thiếu; sinh âm thanh phát âm. \newline
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.0cm}|p{11.4cm}|}
    \hline
    \textbf{Mã use case} & UC-SPEAKING \\ \hline
    \textbf{Tên} & Luyện nói theo kịch bản hội thoại \\ \hline
    \textbf{Tác nhân} & Người học, Groq (LLM hội thoại) \\ \hline
    \textbf{Mô tả} & Người học hội thoại nhập vai theo lượt với AI dựa trên một kịch bản đã xuất bản, được sửa lỗi theo lượt và nhận báo cáo phản hồi khi kết thúc. \\ \hline
    \textbf{Tiền điều kiện} & Người học đã đăng nhập; còn trong hạn mức số phiên luyện nói theo ngày. \\ \hline
    \textbf{Hậu điều kiện} & Toàn bộ lượt hội thoại được lưu lại; phiên có báo cáo phản hồi cuối phiên. \\ \hline
    \textbf{Luồng chính} &
      1.~Người học chọn một kịch bản (kèm tuỳ chọn các từ mục tiêu). \newline
      2.~Hệ thống kiểm tra hạn mức phiên/ngày, chốt bản sao thông số kịch bản + trình độ + từ mục tiêu, lưu câu mở đầu của AI và trả về phiên. \newline
      3.~Mỗi lượt: người học gửi câu nói; hệ thống ghép toàn bộ hội thoại làm ngữ cảnh, gọi Groq để sinh câu trả lời, gợi ý chỉnh lỗi và các từ mục tiêu đã dùng; lưu cả lượt người học và lượt AI. \newline
      4.~Lặp lại bước~3 cho tới khi đạt giới hạn số lượt hoặc người học chủ động kết thúc. \newline
      5.~Khi kết thúc, hệ thống gọi LLM trên toàn bộ hội thoại để sinh báo cáo (tóm tắt, lỗi tiêu biểu, từ mục tiêu dùng/bỏ lỡ, trình độ ước lượng, gợi ý luyện tiếp) và lưu lại. \\ \hline
    \textbf{Luồng ngoại lệ} &
      1a.~Kịch bản không tồn tại hoặc chưa xuất bản $\rightarrow$ lỗi 404. \newline
      2a.~Vượt hạn mức phiên theo ngày $\rightarrow$ lỗi 429. \newline
      3a.~Vượt giới hạn số lượt của phiên $\rightarrow$ lỗi 400 (đề nghị kết thúc). \newline
      3b.~Dịch vụ Groq không khả dụng $\rightarrow$ lỗi 503; lượt vừa rồi không bị lưu hỏng. \newline
      5a.~Sinh báo cáo thất bại $\rightarrow$ đánh dấu báo cáo \code{failed}; người học có thể yêu cầu sinh lại. \\ \hline
  \end{tabular}
  \caption{Đặc tả use case Luyện nói theo kịch bản hội thoại}
  \label{tab:uc-speaking}
\end{table}
...
Phần này mô tả luồng xử lý của bốn nghiệp vụ phức tạp nhất bằng biểu đồ hoạt
...
độ, hệ thống chọn từ, sinh chuỗi câu hỏi cho mỗi từ rồi sắp xếp thành các
\textit{vòng theo dạng câu hỏi} (mỗi dạng trải trên tất cả các từ, theo độ
khó tăng dần); người học trả lời lần lượt; chỉ ở \textit{bước cuối} của mỗi
từ mới cập nhật tiến độ theo SM-2 (các bước trước chỉ chấm để phản hồi).
Trạng thái thẻ chuyển dịch \code{new} $\rightarrow$ \code{learning}
$\rightarrow$ \code{review} $\rightarrow$ \code{mastered} theo kết quả ôn tập.
...
% :Sắp xếp thành vòng theo dạng câu hỏi (type-major, độ khó tăng dần);
% repeat
%   :Hiển thị câu hỏi;
%   :Người học trả lời;
...
từ vựng: API chỉ tạo công việc và trả về ngay, còn việc làm giàu dữ liệu do
worker đảm nhiệm bằng cách điền từng trường từ các nguồn xác định (từ điển,
kho ngữ liệu, dịch máy, bảng CEFR) và chỉ gọi Gemma dự phòng cho trường còn
thiếu.
...
% :Phân giải từ điển (Free Dictionary / Wiktionary);
% :Điền nghĩa/ví dụ (kho ngữ liệu)/dịch (OPUS-MT)/CEFR từ nguồn xác định;
% if (Còn trường trống & bật Gemma fallback?) then (có)
%   :Gọi Gemma 3 bù trường thiếu;
% else (không)
% endif
...
\subsection{Hoạt động một phiên luyện nói theo kịch bản}
\label{subsec:activity-speaking}

Hình~\ref{fig:activity-speaking} mô tả vòng đời một phiên luyện nói. Khác với
luyện viết câu (chấm bất đồng bộ một câu), luyện nói là một vòng lặp hội thoại
\textit{đồng bộ theo lượt}: mỗi lượt gọi LLM một lần với toàn bộ hội thoại làm
ngữ cảnh. Hệ thống ràng buộc bằng hạn mức số phiên/ngày và giới hạn số lượt mỗi
phiên; khi kết thúc, một lời gọi LLM riêng (dùng mô hình mạnh hơn) sinh báo cáo
phản hồi cuối phiên.

% --- PlantUML gợi ý ---------------------------------------------------
% @startuml
% start
% :Người học chọn kịch bản (+ từ mục tiêu tuỳ chọn);
% if (Còn hạn mức phiên/ngày?) then (không)
%   :Trả về 429;
%   stop
% else (còn)
% endif
% :Chốt snapshot kịch bản + trình độ + từ mục tiêu;
% :Lưu câu mở đầu của AI (không gọi LLM);
% repeat
%   :Người học gửi câu nói;
%   if (Vượt giới hạn số lượt?) then (có)
%     :Trả về 400 (đề nghị kết thúc);
%     break
%   else (chưa)
%   endif
%   :Ghép toàn bộ hội thoại làm ngữ cảnh + gọi Groq;
%   if (Groq khả dụng?) then (không)
%     :Trả về 503 (không lưu lượt hỏng);
%   else (có)
%     :Lưu lượt người học + lượt AI (reply, chỉnh lỗi, từ đã dùng);
%   endif
% repeat while (Tiếp tục hội thoại?) is (tiếp)
% ->kết thúc;
% :Đánh dấu phiên ENDED;
% :Gọi LLM (mô hình mạnh hơn) sinh báo cáo trên toàn hội thoại;
% if (Sinh báo cáo thành công?) then (có)
%   :Lưu báo cáo = ready;
% else (không)
%   :Đánh dấu report = failed (cho phép sinh lại);
% endif
% stop
% @enduml
\figph{activity-phien-luyen-noi.png}{0.6}{Biểu đồ hoạt động của một phiên luyện nói theo kịch bản}{fig:activity-speaking}
...
các thành phần cho bốn kịch bản quan trọng.
...
\subsection{Một lượt hội thoại trong phiên luyện nói}
\label{subsec:seq-speaking-turn}

Hình~\ref{fig:seq-speaking-turn} mô tả trình tự xử lý một lượt nói. Dịch vụ
phiên đọc lại toàn bộ các lượt đã lưu để dựng ngữ cảnh, gọi nền tảng Groq sinh
câu trả lời kèm gợi ý chỉnh lỗi (định dạng JSON có cấu trúc), rồi lưu đồng thời
lượt của người học và lượt của AI. Khi Groq lỗi, hệ thống trả về 503 mà không
ghi đè trạng thái hội thoại.

% --- PlantUML gợi ý ---------------------------------------------------
% @startuml
% actor "Người học" as U
% participant "SpeakingSessionController" as C
% participant "SpeakingSessionService" as S
% participant "Groq (Llama 3.3)" as GR
% database "PostgreSQL" as DB
% U -> C : POST /v1/speaking/sessions/:id/turn (text)
% C -> S : takeTurn(userId, sessionId, text)
% S -> DB : SELECT speaking_turns (theo turn_index)
% DB --> S : lịch sử hội thoại
% S -> S : Kiểm tra giới hạn số lượt + dựng ngữ cảnh
% S -> GR : chatCompletion(prompt + lịch sử + từ mục tiêu)
% alt Groq không khả dụng
%   GR --> S : lỗi / timeout
%   S --> C : 503
%   C --> U : 503 (không lưu lượt hỏng)
% else Thành công
%   GR --> S : JSON { reply, corrections, usedTargetWords }
%   S -> DB : INSERT lượt người học + lượt AI
%   S --> C : { reply, corrections, usedTargetWords }
%   C --> U : 200 kết quả lượt
% end
% @enduml
\figph{seq-luot-hoi-thoai.png}{0.95}{Biểu đồ tuần tự một lượt hội thoại trong phiên luyện nói}{fig:seq-speaking-turn}
...
% cloud "Từ điển ngoài (Free Dictionary)" as Dict
% cloud "OPUS-MT (dịch máy)" as OpusMt
% cloud "Gemma 3 (dự phòng, Google AI Studio)" as Gemma
% cloud "Groq (Llama 3.3)" as Groq
...
% API --> OAuth
% API --> Mail
% Worker --> Dict : làm giàu từ vựng
% Worker --> OpusMt : dịch từ / câu
% Worker --> Gemma : dự phòng làm giàu
% Worker --> TTS
% Worker --> CDN
% API --> Pron
% API --> Groq : hội thoại theo lượt + báo cáo
% @enduml
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{2.7cm}|p{8.6cm}|p{2.8cm}|}
    ...
    \hline
    \code{speaking-room} & Phòng luyện nói: quản trị kịch bản (soạn, nháp bằng AI, xuất bản) và điều phối phiên hội thoại theo lượt với Groq, sinh báo cáo cuối phiên. & scenarios, speaking\_sessions, speaking\_turns \\
    \hline
...
(sinh nội dung và chấm điểm câu); \textbf{Groq} chạy các mô hình
\textbf{Llama~3.3} (hội thoại theo lượt trong phòng luyện nói, sinh nháp kịch
bản và báo cáo cuối phiên); \textbf{vi dịch vụ chấm phát âm} (FastAPI, chấm
điểm âm vị); \textbf{Cloudinary} (lưu trữ hình ảnh và âm thanh);
...
Khác với các tác vụ làm giàu và chấm điểm câu (đẩy sang worker nền), hội thoại
luyện nói được gọi \textit{trực tiếp trong luồng API} theo lượt vì người học chờ
phản hồi tức thì; bù lại, hệ thống bảo vệ khoá dùng chung bằng hạn mức số
phiên/ngày và giới hạn số lượt mỗi phiên. Khi một dịch vụ ngoài không khả dụng,
hệ thống suy giảm có kiểm soát bằng mã lỗi 503 thay vì sụp đổ (đáp ứng NFR-04).
...
chiếu bởi tiến độ học, bộ thẻ, bài luyện tập và nhật ký hoạt động. Phân hệ
luyện nói bổ sung một cụm thực thể riêng: \code{scenarios} (kịch bản), mỗi
\code{speaking_sessions} (phiên) gắn với một kịch bản và một người dùng, và
mỗi phiên chứa nhiều \code{speaking_turns} (lượt hội thoại).
...
% scenarios ||--o{ speaking_sessions
% speaking_sessions ||--o{ speaking_turns
% @enduml
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{4.0cm}|p{10.6cm}|}
    ...
    \hline
    scenarios & Kịch bản luyện nói do quản trị viên biên soạn (vai, bối cảnh, mục tiêu, câu mở đầu, trạng thái xuất bản). \\
    \hline
    speaking\_sessions & Một phiên luyện nói của người học theo một kịch bản, kèm bản sao thông số và báo cáo cuối phiên. \\
    \hline
    speaking\_turns & Các lượt hội thoại (người học/AI) trong một phiên, kèm gợi ý chỉnh lỗi và từ mục tiêu đã dùng. \\
    \hline
  \end{tabular}
...
Phân hệ luyện nói bổ sung ba bảng. Bảng~\ref{tab:db-scenarios} mô tả thực thể
kịch bản; Bảng~\ref{tab:db-speaking-sessions} mô tả phiên luyện nói; cấu trúc
lượt hội thoại (\code{speaking_turns}) được tóm tắt cùng các bảng phụ trợ ở
cuối mục.
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.7cm}|p{3.1cm}|p{7.6cm}|}
    ...
    id & uuid (PK) & Khoá chính. \\
    \hline
    title & varchar(160) & Tiêu đề kịch bản. \\
    \hline
    topic & varchar(64) & Nhãn chủ đề để gợi ý (vd. \code{food}, \code{travel}). \\
    \hline
    cefr\_level & enum, null & Trình độ CEFR đích (null = mọi trình độ). \\
    \hline
    setting & text & Bối cảnh hội thoại. \\
    \hline
    ai\_role / user\_role & varchar(120) & Vai của AI và vai của người học. \\
    \hline
    goal & text & Mục tiêu giao tiếp của phiên. \\
    \hline
    opening\_line & text & Câu mở đầu AI nói trước (không cần gọi LLM). \\
    \hline
    seed\_phrases & text[] & Cụm từ gợi ý nhúng vào hội thoại. \\
    \hline
    est\_turns & smallint, null & Số lượt ước tính. \\
    \hline
    intro\_video\_script / intro\_video\_url & text / varchar(512), null & Kịch bản và liên kết video giới thiệu (sinh một lần, dùng lại). \\
    \hline
    status & enum & \code{draft} / \code{published} / \code{retired}. \\
    \hline
    version & int & Tăng mỗi lần sửa kịch bản đã xuất bản (để phiên đang chạy giữ nguyên thông số). \\
    \hline
    created\_by & uuid (FK), null & Quản trị viên biên soạn (\code{SET NULL} khi xoá). \\
    \hline
    created\_at / updated\_at & timestamptz & Thời điểm tạo / cập nhật. \\
    \hline
  \end{tabular}
  \caption{Cấu trúc bảng \code{scenarios}}
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.7cm}|p{3.1cm}|p{7.6cm}|}
    ...
    user\_id & uuid (FK) & Người học (\code{CASCADE} khi xoá người dùng). \\
    \hline
    scenario\_id & uuid (FK) & Kịch bản được luyện (\code{CASCADE} khi xoá kịch bản). \\
    \hline
    scenario\_version & int & Phiên bản kịch bản lúc bắt đầu phiên. \\
    \hline
    scenario\_snapshot & jsonb & Bản sao các trường văn bản của kịch bản, cố định trong suốt phiên. \\
    \hline
    cefr\_level & enum, null & Trình độ dùng để điều chỉnh ngôn ngữ của AI (chốt từ hồ sơ). \\
    \hline
    selected\_vocabulary\_ids & uuid[] & Các từ mục tiêu người học chọn luyện. \\
    \hline
    selected\_words & text[] & Bản sao lemma của các từ mục tiêu (ổn định, rẻ cho mỗi lượt). \\
    \hline
    status & enum & \code{active} / \code{ended}. \\
    \hline
    report\_status & enum & \code{pending} / \code{ready} / \code{failed}. \\
    \hline
    report & jsonb, null & Báo cáo phản hồi cuối phiên. \\
    \hline
    report\_model & varchar(64), null & Mô hình sinh báo cáo (vd. \code{llama-3.3-70b}). \\
    \hline
    ended\_at & timestamptz, null & Thời điểm kết thúc phiên. \\
    \hline
    created\_at / updated\_at & timestamptz & Thời điểm tạo / cập nhật. \\
    \hline
  \end{tabular}
  \caption{Cấu trúc bảng \texttt{speaking\_sessions}}
  \label{tab:db-speaking-sessions}
\end{table}
...
\begin{itemize}
  ...
  \item \code{speaking_turns}: \code{id}, \code{session_id} (FK,
        \code{CASCADE}), \code{turn_index} (vị trí lượt, câu mở đầu của AI là
        0), \code{role} (\code{ai}/\code{user}), \code{text} (nội dung lượt),
        \code{corrections} (jsonb gợi ý chỉnh lỗi, gắn vào lượt AI),
        \code{used_target_words} (text[]); duy nhất theo (\code{session_id},
        \code{turn_index}).
\end{itemize}
...
cung cấp khoảng 75 endpoint; danh sách đầy đủ được quản lý trong tài liệu
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.6cm}|p{4.6cm}|p{2.2cm}|p{3.4cm}|}
    ...
    \hline
    Luyện nói & \code{/v1/speaking} & JWT & Duyệt kịch bản, phiên hội thoại theo lượt, báo cáo cuối phiên. \\
    \hline
    Bảng xếp hạng & \code{/v1/leaderboard} & JWT & Xếp hạng cộng đồng. \\
    \hline
    Quản trị & \code{/v1/admin/*} & JWT (admin) & Quản trị từ vựng, chủ đề, người dùng, kịch bản luyện nói, duyệt AI. \\
    \hline
  \end{tabular}
...
Trọng tâm trình bày của báo cáo là backend (mục~\ref{sec:muc-tieu}), nhưng
hệ thống còn có một ứng dụng giao diện người dùng hoàn chỉnh tiêu thụ các API
đó (hiện thực hoá ở mục~\ref{sec:hien-thuc-frontend},
Chương~\ref{chuong:hien-thuc-hoa}). Phần này trình bày \textit{thiết kế} giao
diện: hệ thống thiết kế, bản đồ điều hướng, phác thảo và ảnh chụp các màn
hình chính, và ánh xạ màn hình--API.

\subsection{Hệ thống thiết kế}
\label{subsec:design-system}

Giao diện được dựng trên một \textbf{hệ thống thiết kế} thống nhất: bộ
\textit{biến thiết kế} (design token) khai báo bằng biến CSS theo không gian
màu đồng đều cảm nhận \textbf{OKLCH}~\cite{w3ccsscolor4}, với bảng màu sáng/tối
song song (theme-ready) và một thang \textit{bo góc} suy theo bội số từ một
giá trị gốc để đồng bộ toàn hệ thống. Bộ thành phần dùng
\textbf{shadcn/ui}~\cite{shadcnui2024} trên các primitive khả truy cập của
\textbf{Base UI}~\cite{baseui2024}, tạo kiểu bằng \textbf{Tailwind CSS
v4}~\cite{tailwind2024}.

Về \textit{nhận diện thương hiệu}, khu vực học của người dùng mang bộ token
"Sprout" (tông bạc hà) được \textit{giới hạn phạm vi} vào lớp \code{.app-shell}
của nhóm tuyến \code{(app)} --- các primitive shadcn dùng lại tự kế thừa tông
màu này --- trong khi khu vực quản trị giữ \textit{chủ đề trung tính} mặc định
để tách biệt công cụ vận hành khỏi trải nghiệm học. Về \textit{kiểu chữ}, ứng
dụng tự lưu trữ phông qua \code{next/font}: Geist cho nền chung, Plus Jakarta
Sans cho văn bản giao diện khu vực học và Newsreader (serif) cho các "khoảnh
khắc trình bày" như con số lớn và lời chào. Bảng~\ref{tab:design-system} tóm tắt
các trụ cột của hệ thống thiết kế.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.4cm}|p{11.0cm}|}
    \hline
    \textbf{Trụ cột} & \textbf{Lựa chọn thiết kế} \\
    \hline
    Màu sắc & Biến CSS theo không gian OKLCH; bảng màu sáng/tối song song (theme-ready). \\
    \hline
    Bo góc \& khoảng cách & Thang bo góc suy theo bội số từ một giá trị gốc, đồng bộ toàn hệ thống. \\
    \hline
    Thành phần & shadcn/ui trên primitive khả truy cập Base UI; tạo kiểu bằng Tailwind CSS v4. \\
    \hline
    Thương hiệu & Token "Sprout" (bạc hà) giới hạn trong \code{.app-shell} của khu vực học; quản trị dùng chủ đề trung tính. \\
    \hline
    Kiểu chữ & Geist (nền chung), Plus Jakarta Sans (giao diện học), Newsreader serif (số/lời chào), tự lưu trữ qua \code{next/font}. \\
    \hline
  \end{tabular}
  \caption{Các trụ cột của hệ thống thiết kế giao diện}
...
Hình~\ref{fig:ui-sitemap} mô tả cấu trúc điều hướng của ứng dụng khách. Ứng
dụng chia thành hai khu vực theo nhóm tuyến: khu vực \textit{người học}
(\code{(app)}) và khu vực \textit{quản trị} (\code{(admin)}). Từ màn hình đăng
nhập, người dùng mới đi qua luồng khai báo hồ sơ rồi tới trang chủ; từ trang
chủ có thể truy cập các nhánh học, khám phá từ vựng/bộ thẻ, từ vựng cá nhân,
luyện tập (viết câu, phát âm và phòng luyện nói), cộng đồng và thiết lập. Quản
trị viên đi vào khu vực quản trị riêng gồm quản trị từ vựng, chủ đề, kịch bản
luyện nói và người dùng.
...
% "Trang chủ (Dashboard)" --> "Từ vựng cá nhân"
% "Trang chủ (Dashboard)" --> "Luyện viết câu"
% "Trang chủ (Dashboard)" --> "Luyện phát âm"
% "Trang chủ (Dashboard)" --> "Phòng luyện nói (chọn kịch bản -> hội thoại -> báo cáo)"
% "Trang chủ (Dashboard)" --> "Bảng xếp hạng / Cộng đồng"
% "Trang chủ (Dashboard)" --> "Hồ sơ cá nhân & Thiết lập"
% "Đăng nhập / Đăng ký" --> "Khu vực quản trị (Admin)"
% "Khu vực quản trị (Admin)" --> "Quản trị từ vựng (soạn / duyệt AI / nhập hàng loạt)"
% "Khu vực quản trị (Admin)" --> "Quản trị chủ đề"
% "Khu vực quản trị (Admin)" --> "Quản trị kịch bản luyện nói (soạn / nháp AI / xuất bản)"
% "Khu vực quản trị (Admin)" --> "Quản trị người dùng"
% @enduml
\figph{ui-sitemap.png}{0.9}{Bản đồ điều hướng của ứng dụng khách}{fig:ui-sitemap}

\subsection{Phác thảo và ảnh chụp các màn hình chính}
...
Để minh hoạ kết quả hiện thực hoá, các hình tiếp theo là ảnh chụp từ ứng dụng
đã hoàn thiện, được nhóm theo từng khu vực chức năng.

\paragraph{Trải nghiệm người học.} Hình~\ref{fig:ui-dashboard} là trang chủ
của người học mang nhận diện Sprout (chuỗi ngày học, mục tiêu ngày, vườn từ
vựng); Hình~\ref{fig:ui-explore} là màn hình khám phá từ vựng và bộ thẻ cộng
đồng. Một phiên học diễn ra qua Hình~\ref{fig:ui-learn-session} (giao diện
phiên học), Hình~\ref{fig:ui-question} (một dạng câu hỏi) và
Hình~\ref{fig:ui-learn-result} (kết quả cuối phiên). Hai chế độ luyện tập chủ
động là luyện viết câu (Hình~\ref{fig:ui-practice-write}) và luyện phát âm có
chấm điểm (Hình~\ref{fig:ui-practice-speak}).

\figph{homepage.jpg}{0.85}{Màn hình trang chủ của người học}{fig:ui-dashboard}

\figph{explore_commmunity_list_vocabulary.jpg}{0.85}{Màn hình khám phá từ vựng và bộ thẻ cộng đồng}{fig:ui-explore}

\figph{learnsession.jpg}{0.85}{Giao diện một phiên học}{fig:ui-learn-session}

\figph{question_per_sentence.jpg}{0.85}{Một dạng câu hỏi trong phiên học}{fig:ui-question}

\figph{learning_session_result.jpg}{0.85}{Màn hình kết quả cuối phiên học}{fig:ui-learn-result}

\figph{practice_vocabulary.jpg}{0.85}{Màn hình luyện viết câu có chấm điểm}{fig:ui-practice-write}

\figph{pronunciation_score.jpg}{0.85}{Màn hình luyện phát âm và điểm phát âm}{fig:ui-practice-speak}

\paragraph{Bộ thẻ, cộng đồng và xếp hạng.} Người học tự tạo bộ thẻ từ vựng
(Hình~\ref{fig:ui-create-deck}), chia sẻ bộ thẻ ra cộng đồng
(Hình~\ref{fig:ui-share-deck}) và theo dõi vị trí của mình trên bảng xếp hạng
(Hình~\ref{fig:ui-leaderboard}).

\figph{create_vocabulary_list.jpg}{0.85}{Màn hình tạo bộ thẻ từ vựng cá nhân}{fig:ui-create-deck}

\figph{share_list_to_community.jpg}{0.85}{Chia sẻ bộ thẻ ra cộng đồng}{fig:ui-share-deck}

\figph{leader_board.jpg}{0.85}{Màn hình bảng xếp hạng cộng đồng}{fig:ui-leaderboard}

\paragraph{Khu vực quản trị.} Hình~\ref{fig:ui-admin-dashboard} là trang chủ
khu vực quản trị; Hình~\ref{fig:ui-admin-vocab} là trình soạn từ vựng;
Hình~\ref{fig:ui-admin-review} là hàng đợi duyệt bản nháp do AI sinh; và
Hình~\ref{fig:ui-admin-scenario} là trình soạn kịch bản luyện nói.

\figph{admin_dashboard.jpg}{0.85}{Trang chủ khu vực quản trị}{fig:ui-admin-dashboard}

\figph{admin_create_vocabulary.jpg}{0.85}{Trình soạn từ vựng trong khu vực quản trị}{fig:ui-admin-vocab}

\figph{review_draft.jpg}{0.85}{Hàng đợi duyệt nội dung do AI sinh}{fig:ui-admin-review}

\figph{admin_create_scenario_for_speaking.jpg}{0.85}{Trình soạn kịch bản luyện nói trong khu vực quản trị}{fig:ui-admin-scenario}
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.6cm}|p{11.0cm}|}
    ...
    \hline
    Phòng luyện nói & \code{GET /v1/speaking/scenarios}, \code{POST /v1/speaking/sessions}, \code{POST /v1/speaking/sessions/:id/turn}, \code{POST /v1/speaking/sessions/:id/end} \\
    \hline
    Bảng xếp hạng / Cộng đồng & \code{GET /v1/leaderboard}, \code{GET /v1/decks/public} \\
    \hline
    Từ vựng cá nhân & \code{/v1/me/vocabularies}, \code{POST /v1/me/vocabularies/quick-create}, \code{GET /v1/me/vocabularies/jobs/:id} \\
    \hline
    Bộ thẻ cá nhân & \code{/v1/me/decks}, \code{/v1/decks} \\
    \hline
    Hồ sơ \& thiết lập & \code{GET /v1/auth/me}, \code{PATCH /v1/users/:id} \\
    \hline
    Quản trị từ vựng & \code{/v1/admin/vocabularies}, \code{/v1/admin/vocabularies/:id}, \code{/v1/admin/vocabularies/quick}, \code{POST /v1/admin/vocabularies/bulk-import} \\
    \hline
    Quản trị kịch bản luyện nói & \code{/v1/admin/scenarios}, \code{POST /v1/admin/scenarios/draft}, \code{POST /v1/admin/scenarios/:id/publish} \\
    \hline
    Quản trị chủ đề / người dùng & \code{/v1/admin/topics}, \code{/v1/admin/users} \\
    \hline
  \end{tabular}
...
Chương này đã phân tích đầy đủ yêu cầu chức năng (29 yêu cầu chức năng) và phi
...
tầng xử lý nền bất đồng bộ, thiết kế cơ sở dữ liệu gồm 20 bảng quanh thực thể
từ vựng (kèm cụm thực thể luyện nói: kịch bản, phiên và lượt hội thoại), bộ
API REST có phiên bản hoá và phác thảo giao diện người dùng bám sát API. Đây là
cơ sở để chương tiếp theo trình bày quá trình hiện thực hoá và kiểm thử hệ thống.
