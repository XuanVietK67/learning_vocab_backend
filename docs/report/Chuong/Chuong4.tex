...
Sau mỗi thao tác \textit{ghi} (qua Server Action), giao diện được làm tươi bằng
cách \textit{vô hiệu hoá bộ nhớ đệm} của đúng những tuyến bị ảnh hưởng:
\code{revalidatePath} đánh dấu trang danh sách và trang chi tiết cần kết xuất
lại ở lần truy cập kế tiếp, nhờ đó dữ liệu hiển thị luôn nhất quán với máy chủ
mà không cần quản lý trạng thái phía máy khách. Những phần dữ liệu chậm được
bọc trong ranh giới \textit{Suspense} (\code{loading.tsx}) để truyền dòng
(streaming) giao diện khung trước, rồi mới điền dữ liệu.
...
phía máy khách. Biểu mẫu được nối với Server Action bằng các hook của
\textbf{React~19}: \code{useActionState} giữ trạng thái trả về của hành động
(cờ \code{ok}, thông báo lỗi, lỗi theo từng trường và giá trị đã nhập để điền
lại), còn \code{useFormStatus} điều khiển trạng thái chờ của nút gửi. Cùng một
lược đồ \code{zod} trong \code{src/lib/validations/} được dùng để kiểm tra ở
Server Action; lỗi đầu tiên được ánh xạ về đúng trường để hiển thị nội tuyến
kèm thuộc tính khả truy cập (\code{aria-invalid}, \code{aria-describedby}). Một
thành phần bao mỏng \code{ActionForm} (dùng ở khu vực quản trị) đóng gói khuôn
mẫu này: tự đặt lại biểu mẫu khi thành công, hiện thông báo nổi (toast) và phát
tín hiệu để thu gọn trình soạn nội tuyến. Vì kiểm tra nằm ở phía máy chủ, biểu
mẫu vẫn an toàn ngay cả khi JavaScript phía máy khách chưa kịp tải.
