...
\begin{table}[h]
  ...
  \begin{tabular}{|p{2.6cm}|p{4.4cm}|p{6.6cm}|}
    ...
    \hline
    \code{speaking-room} & \code{src/speaking-room} & \code{speaking-session.service.ts} (điều phối phiên), \code{speaking-turn.ts} (một lượt hội thoại), \code{session-report.ts} (báo cáo cuối phiên), \code{scenario-draft.ts}, \code{scenarios.service.ts}. \\
    \hline
...
Tầng dữ liệu hiện thực 20 bảng trong Bảng~\ref{tab:db-overview} thành các lớp
...
đã ký $\rightarrow$ sắp xếp thành các vòng theo dạng câu hỏi
(mục~\ref{subsec:impl-rounds}) $\rightarrow$ chấm điểm $\rightarrow$ (ở bước
cuối) lập lịch lại.
...
(hằng ngày, theo chủ đề, theo bộ thẻ, ôn tập); riêng hai chế độ theo chủ đề
và theo bộ thẻ có thêm \textit{cờ luyện lại} (practice) --- khi bật, điều kiện
đến hạn \code{next_review_at <= now} được bỏ qua để người học luyện lại ngay
các từ đã học mà không cần chờ tới hạn. Nguyên tắc chung là \textit{ưu tiên
từ đã đến hạn} (sắp theo \code{next_review_at} tăng dần), sau đó bù thêm
\textit{từ mới} cho đủ hạn mức phiên. Điểm "thông minh" nằm ở
...
\subsection{Sắp xếp phiên học theo vòng dạng câu hỏi}
\label{subsec:impl-rounds}

Sau khi mỗi từ đã được mở rộng thành chuỗi câu hỏi riêng (word-major),
\code{LearnService} \textit{sắp xếp lại} toàn bộ phiên thành các \textbf{vòng
theo dạng câu hỏi} (type-major rounds, hàm \code{toTypeMajorRounds}): người
học trả lời \textit{một dạng câu hỏi trên tất cả các từ} rồi mới sang dạng kế
tiếp, thay vì làm hết các câu của một từ rồi mới qua từ khác. Cách trình bày
này giúp phiên học bớt đơn điệu và gom các thao tác cùng loại lại với nhau.
Việc sắp xếp tuân theo ba ràng buộc để không phá vỡ logic học:

\begin{itemize}
  \item \textbf{Tăng dần theo độ khó:} các vòng chạy theo thứ tự dải khó tăng
        dần (mục~\ref{subsec:impl-questions}), nên mỗi từ vẫn gặp câu hỏi của
        nó từ dễ đến khó và \textit{bước khó nhất --- bước mang sự kiện SRS ---
        luôn nằm ở vòng cuối}.
  \item \textbf{Xáo trộn trong mỗi vòng:} trong một vòng, thứ tự các từ được
        \textit{xáo trộn xác định} theo một hạt giống riêng của vòng, nên thứ
        tự khác nhau giữa các vòng nhưng vẫn tái lập được.
  \item \textbf{Giữ nguyên "phong bì" của mỗi câu:} mỗi mục vẫn mang
        \code{groupId}, \code{stepIndex}, \code{stepCount} và chữ ký HMAC của
        nó, nên việc chấm điểm và lập lịch lại ở bước cuối
        (mục~\ref{subsec:impl-hmac}) không bị ảnh hưởng bởi thứ tự trình bày.
\end{itemize}
...
Chương~\ref{chuong:tong-quan}. Phần lớn các tác vụ này đều nặng và phụ thuộc
dịch vụ ngoài, nên được hiện thực trên một \textit{hạ tầng xử lý nền} chung;
riêng phòng luyện nói (mục~\ref{subsec:impl-speaking}) gọi LLM \textit{đồng bộ}
theo lượt vì người học chờ phản hồi tức thì.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.0cm}|p{4.6cm}|p{6.4cm}|}
    ...
    \hline
    Làm giàu từ vựng & Tạo nhanh / nhập hàng loạt & Tra từ điển, truy hồi ví dụ từ kho ngữ liệu, dịch và gắn CEFR từ các nguồn xác định (LLM chỉ là dự phòng); tạo bản ghi từ vựng. \\
    \hline
...
\paragraph{Lưu ý về mô hình ngôn ngữ.} Khi cần đến LLM --- nay chủ yếu là
đường \textit{dự phòng} của pipeline làm giàu (mục~\ref{subsec:impl-enrich})
và việc chấm điểm câu luyện tập (mục~\ref{subsec:impl-judge}) --- các lời gọi
được thực hiện qua API \code{generateContent} của nền tảng \textbf{Google AI
Studio}, dùng họ mô hình mở \textbf{Gemma} của Google
DeepMind~\cite{gemma2025technical} làm nền tảng tham chiếu. Vì mô hình được cấu hình qua biến môi trường
...
\subsection{Làm giàu dữ liệu từ vựng tự động từ nhiều nguồn}
\label{subsec:impl-enrich}

Đây là tính năng cốt lõi của tạo nhanh từ vựng: từ một \textit{lemma} đơn lẻ,
hệ thống tự dựng nên một bản ghi từ vựng đầy đủ (use case ở
Bảng~\ref{tab:uc-quickcreate}). Nguyên tắc thiết kế hiện tại là \textbf{ưu
tiên các nguồn dữ liệu xác định (deterministic) thay vì sinh bằng LLM}: mỗi
trường được lấy từ một nguồn tra cứu hoặc truy hồi chuyên trách, còn
\textbf{Gemma chỉ còn đóng vai trò dự phòng} cho những trường mà không nguồn
nào phủ được. Cơ chế dự phòng này nằm sau một cờ cấu hình
(\code{enrichment.useGemmaFallback}): khi tắt, pipeline chạy \textit{hoàn toàn
không gọi Gemma} và để trống các trường còn thiếu cho người dùng tự sửa, thay
vì sinh nội dung không chắc chắn.

\code{EnrichmentProcessor} điều phối các nguồn theo
Bảng~\ref{tab:enrich-sources}. Trước hết nó \textit{phân giải mục từ điển}:
tiếng Anh tra Free Dictionary API~\cite{freedictionaryapi}, các ngôn ngữ khác
--- hoặc khi API trên không có mục --- tra tập dữ liệu từ điển đa ngữ
\textbf{Wiktionary} (qua wiktextract~\cite{ylonen2022wiktextract}) đã nạp sẵn
trong cơ sở dữ liệu. Khi có mục từ điển, các trường còn lại được điền từ các
nguồn xác định riêng; chỉ khi \textit{cả hai} từ điển đều không có mục (đường
\textit{from-scratch}) và cờ dự phòng đang bật, worker mới gọi Gemma sinh toàn
bộ cấu trúc nghĩa --- nếu cờ tắt, công việc kết thúc không tạo bản ghi (đánh
dấu thất bại) thay vì âm thầm gọi Gemma.

\begin{table}[h]
  \centering
  \small
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{|p{2.9cm}|p{7.0cm}|p{4.0cm}|}
    \hline
    \textbf{Trường} & \textbf{Nguồn xác định (không LLM)} & \textbf{Gemma (chỉ khi dự phòng bật)} \\
    \hline
    Từ loại, định nghĩa, đồng/trái nghĩa & Từ điển (Free Dictionary API cho tiếng Anh; Wiktionary đa ngữ) & --- \\
    \hline
    Phiên âm IPA & Từ điển; ghép IPA theo từng từ cho cụm từ; G2P \code{espeak-ng}~\cite{espeakng} làm phương án cuối & IPA "best-effort" khi các nguồn trên đều trống \\
    \hline
    Câu ví dụ & Truy hồi từ kho ngữ liệu (\code{corpus_sentence} dựng từ Tatoeba~\cite{tatoeba}/OpenSubtitles) bằng tìm kiếm toàn văn khớp dạng biến cách, xếp theo điểm GDEX & Sinh ví dụ khi kho ngữ liệu không đủ \\
    \hline
    Bản dịch từ/nghĩa & Tra từ điển song ngữ (\code{bilingual_lexicon}) $\rightarrow$ dịch máy OPUS-MT~\cite{tiedemann2020opusmt} rồi ghi đệm lại & Bản dịch theo nghĩa khi hai nguồn trên trống \\
    \hline
    Bậc CEFR & Tra bảng từ vựng CEFR xác định (\code{cefr_lexicon}) theo lemma + từ loại & Ước lượng CEFR khi không có trong bảng \\
    \hline
    Gloss (nhãn ngắn) & Trích cơ học vài từ đầu của định nghĩa (\code{deriveGloss}) & Gloss khi mô hình trả về \\
    \hline
  \end{tabular}
  \caption{Nguồn dữ liệu của từng trường khi làm giàu từ vựng và vai trò dự phòng của Gemma}
  \label{tab:enrich-sources}
\end{table}

Câu ví dụ lấy từ kho ngữ liệu được dịch sang ngôn ngữ đích bằng một lời gọi
OPUS-MT theo lô cho mỗi từ. Mọi kết quả --- bất kể nguồn --- đều đi qua bộ nhớ
đệm làm giàu (\code{enrichment-cache}) trước khi lưu, nên một từ lặp lại không
tốn thêm lời gọi mạng nào; worker còn ghi nhật ký \textit{tỉ lệ phủ} của từng
nguồn xác định để đo mức phụ thuộc còn lại vào Gemma trước khi gỡ bỏ hẳn.

\paragraph{Phân tích phòng thủ trên đường dự phòng Gemma.} Khi đường dự phòng
được dùng, vì Gemma trên AI Studio không hỗ trợ \code{responseSchema} hay vai
trò \textit{hệ thống} (system role), lược đồ JSON mong muốn được \textbf{nhúng
thẳng vào prompt} và phản hồi được \textbf{phân tích phòng thủ}: bóc bỏ dấu rào
```\code{```json}```, dung thứ văn xuôi thừa, kẹp độ dài các trường, và ---
quan trọng nhất --- \textit{từ chối} một CEFR sai/thiếu hay một nghĩa có dưới
hai ví dụ bằng cách ném lỗi để worker \textit{thử lại} thay vì lưu dữ liệu rác:
...
\paragraph{Vòng kiểm soát chất lượng.} Kết quả làm giàu luôn được kiểm soát theo
...
Hình~\ref{fig:qc-confirm}--\ref{fig:qc-list} minh hoạ luồng tạo nhanh và làm
giàu từ vựng từ phía người dùng: xác nhận danh sách từ cần làm giàu
(Hình~\ref{fig:qc-confirm}), tiến trình xử lý nền sau khi gửi
(Hình~\ref{fig:qc-processing}), thông báo hoàn tất (Hình~\ref{fig:qc-finish})
và danh sách từ vừa được tạo (Hình~\ref{fig:qc-list}).

\figph{confirm_word_to_enrich.jpg}{0.85}{Xác nhận danh sách từ cần làm giàu}{fig:qc-confirm}

\figph{processing_enrich_vocabulary.jpg}{0.85}{Tiến trình làm giàu từ vựng xử lý nền}{fig:qc-processing}

\figph{finish_quick_create_vocabulary.jpg}{0.85}{Thông báo hoàn tất tạo nhanh từ vựng}{fig:qc-finish}

\figph{list_vocabulary_just_created.jpg}{0.85}{Danh sách từ vựng vừa được tạo}{fig:qc-list}
...
\subsection{Hội thoại luyện nói theo kịch bản bằng LLM}
\label{subsec:impl-speaking}

Phòng luyện nói (use case ở Bảng~\ref{tab:uc-speaking}) là tính năng AI duy
nhất chạy \textit{đồng bộ trong luồng API} thay vì qua worker nền: một cuộc
hội thoại theo lượt đòi hỏi phản hồi tức thì nên không thể chờ thăm dò. Hệ
thống dùng nền tảng \textbf{Groq} với các mô hình \textbf{Llama~3.3}
\cite{grattafiori2024llama3} và chia tính năng thành ba khối: soạn kịch bản
(quản trị), vòng lặp hội thoại theo lượt, và sinh báo cáo cuối phiên.

\paragraph{Bản sao kịch bản và khoá phiên bản.} Khi người học bắt đầu một
phiên (\code{SpeakingSessionService.start}), hệ thống \textit{chốt} một bản
sao (\code{scenario_snapshot}, kiểu \code{jsonb}) các trường văn bản của kịch
bản cùng trình độ CEFR và danh sách \textit{từ mục tiêu} đã được giải
nghĩa thành lemma. Nhờ vậy, dù quản trị viên sửa kịch bản (làm tăng cột
\code{version}) hay người dùng đổi hồ sơ \textit{giữa chừng}, cuộc hội thoại
đang diễn ra vẫn giữ nguyên thông số nó khởi đầu. Từ mục tiêu được lọc theo
đúng quy tắc khả dụng của phân hệ luyện tập (từ hệ thống đã duyệt hoặc từ
riêng của người học); các từ không hợp lệ bị loại và báo lại cho máy khách.
Lượt~0 lưu sẵn \textit{câu mở đầu} của AI lấy thẳng từ kịch bản --- không tốn
một lời gọi LLM nào.

\paragraph{Một lượt hội thoại.} Mỗi lượt (\code{takeTurn},
Hình~\ref{fig:seq-speaking-turn}) đọc lại toàn bộ các lượt đã lưu, ghép thành
\textit{ngữ cảnh} rồi gọi Groq \textit{một lần}. Mô hình được yêu cầu trả về
\textbf{JSON có cấu trúc} gồm: \code{reply} (câu AI nói tiếp), \code{corrections}
(danh sách gợi ý chỉnh lỗi cho lượt vừa rồi của người học) và
\code{usedTargetWords} (các từ mục tiêu AI đã khéo léo đưa vào). Cả lượt của
người học lẫn lượt của AI được lưu vào \code{speaking_turns} theo \code{turn_index}
tăng dần. Nếu Groq lỗi hoặc hết thời gian chờ, hệ thống trả \code{503} và
\textit{không} ghi lượt hỏng, nên người học có thể thử lại mà không làm rối
mạch hội thoại.

\paragraph{Bảo vệ tài nguyên dùng chung.} Vì gọi LLM đồng bộ trên khoá dùng
chung, tính năng được ràng buộc hai lớp hạn mức: \textit{số phiên theo ngày}
cho mỗi người dùng (kiểm ở \code{start}) và \textit{số lượt tối đa mỗi phiên}
(kiểm ở \code{takeTurn}); vượt lần lượt trả về \code{429} và \code{400}. Để
chịu lỗi hạn mức phía nhà cung cấp, lớp gọi Groq được cấu hình \textit{nhiều
khoá API} và \textit{xoay vòng khoá} khi gặp 429/503 --- cùng cơ chế đã áp
dụng để tăng độ bền cho pipeline làm giàu bằng Gemma.

\paragraph{Báo cáo cuối phiên.} Khi kết thúc (\code{end}), hệ thống chuyển
phiên sang \code{ended} rồi gọi LLM \textit{một lần} trên toàn bộ hội thoại
--- lần này dùng \textit{mô hình mạnh hơn} mô hình hội thoại để ưu tiên chất
lượng đánh giá --- sinh báo cáo gồm: tóm tắt khích lệ, các lỗi tiêu biểu kèm
cách nói tốt hơn, danh sách từ mục tiêu \textit{đã dùng}/\textit{bỏ lỡ},
trình độ CEFR ước lượng và gợi ý luyện tiếp. Quá trình sinh báo cáo
\textbf{idempotent} và \textit{không bao giờ ném lỗi ra ngoài}: thất bại được
ghi nhận là \code{report_status = failed} để máy khách có thể yêu cầu sinh
lại bằng một lời gọi \code{GET} mà không làm phiên kẹt ở trạng thái hỏng.

\paragraph{Soạn kịch bản có AI trợ giúp.} Ở khu vực quản trị, ngoài thao tác
CRUD kịch bản thông thường, một điểm cuối \textit{nháp bằng AI}
(\code{POST /v1/admin/scenarios/draft}) nhận một mô tả ngắn rồi gọi Groq sinh
một bản nháp kịch bản đầy đủ (vai, bối cảnh, mục tiêu, câu mở đầu, cụm từ gợi
ý) \textit{chưa lưu} để quản trị viên xem lại, chỉnh sửa rồi mới tạo. Kịch bản
chỉ hiển thị cho người học sau khi được \textit{xuất bản}
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.6cm}|p{4.2cm}|p{6.0cm}|}
    ...
    \hline
    Tầng đọc dữ liệu & \code{src/lib/me/} & Các hàm đọc bọc \code{cache()} của React (\code{getStats}, \code{getMe}\ldots) chạy phía máy chủ. \\
    \hline
    Luyện tập chủ động & \code{src/app/(app)/practice/} & Màn hình luyện viết câu (thăm dò kết quả bất đồng bộ), luyện phát âm và phòng luyện nói (hội thoại theo lượt). \\
    \hline
    Khu vực quản trị & \code{src/app/(admin)/admin/} & Trình soạn từ vựng, hàng đợi duyệt nội dung AI, nhập/làm giàu hàng loạt. \\
    \hline
    Thư viện thành phần & \code{src/components/} (\code{ui}, \code{app}, \code{admin}, \code{dashboard}) & Thành phần giao diện dùng lại ở nhiều tuyến. \\
    \hline
  \end{tabular}
...
Cả hai chuẩn hoá thân lỗi của NestJS thành một kiểu \code{ApiResult} thống
nhất (cờ \code{ok}, mã trạng thái, dữ liệu, thân lỗi), nên tầng gọi xử lý lỗi
theo \textit{giá trị trả về} thay vì ngoại lệ.
...
độc lập. Thứ tự câu hỏi trong hàng đợi do máy chủ quyết định --- đã sắp thành
các vòng theo dạng câu hỏi (mục~\ref{subsec:impl-rounds}) --- nên máy khách
chỉ tiêu thụ đúng thứ tự nhận được. Nó nắm danh sách câu hỏi còn lại (câu đang
hiển thị luôn là
...
\subsection{Lấy dữ liệu phía máy chủ và vô hiệu hoá bộ nhớ đệm}
\label{subsec:fe-data}

Phần lớn màn hình là Server Component \textit{đọc dữ liệu ngay trên máy chủ}
qua các hàm trong \code{src/lib/me/}. Mỗi hàm đọc được bọc bằng \code{cache()}
của React nên kết quả được \textit{ghi nhớ trong phạm vi một lần kết xuất}:
chẳng hạn \code{getMe} được gọi ở bố cục để bảo vệ tuyến rồi được các trang con
gọi lại mà không phát sinh thêm lượt gọi backend. Các hàm này có hợp đồng
\textit{một lần thử, không tự làm mới} và trả về \code{null} khi chưa đăng nhập
hoặc lỗi, nên an toàn khi chạy trong lúc kết xuất (không ném ngoại lệ làm hỏng
trang):

\begin{verbatim}
// src/lib/me/stats.ts — đọc thống kê trang chủ, ghi nhớ theo mỗi yêu cầu
export const getStats = cache(async (): Promise<StatsResponse | null> => {
  const token = await getAccessToken();
  if (!token) return null;
  const res = await apiRequest<StatsResponse>(
    '/v1/me/stats', { method: 'GET' }, token);
  return res.ok ? res.data : null;
});
\end{verbatim}

Sau mỗi thao tác \textit{ghi} (qua Server Action), \code{revalidatePath} vô
hiệu hoá bộ nhớ đệm của đúng các tuyến bị ảnh hưởng để dữ liệu luôn nhất quán
với máy chủ mà không cần quản lý trạng thái phía máy khách; phần dữ liệu chậm
được bọc trong ranh giới \textit{Suspense} (\code{loading.tsx}) để truyền dòng
(streaming) giao diện khung trước.

\subsection{Biểu mẫu với Server Action và kiểm tra bằng \code{zod}}
\label{subsec:fe-forms}

Mọi thao tác ghi đều đi qua một \textbf{Server Action} thay vì điểm cuối API
phía máy khách, nối với biểu mẫu bằng các hook \textbf{React~19}
(\code{useActionState} giữ trạng thái trả về và lỗi theo từng trường,
\code{useFormStatus} cho trạng thái chờ của nút gửi). Cùng lược đồ \code{zod}
trong \code{src/lib/validations/} kiểm tra ở Server Action; lỗi được ánh xạ về
đúng trường kèm thuộc tính khả truy cập (\code{aria-invalid},
\code{aria-describedby}). Một thành phần bao mỏng \code{ActionForm} đóng gói
khuôn mẫu này (tự đặt lại biểu mẫu, hiện toast khi thành công). Vì kiểm tra
nằm phía máy chủ, biểu mẫu an toàn ngay cả khi JavaScript phía máy khách chưa
tải.

\subsection{Bảo vệ tuyến bằng bố cục thay vì middleware}
\label{subsec:fe-guard}

Hệ thống \textit{không} dùng tệp \code{middleware.ts}; việc kiểm soát truy cập
được đặt ngay trong các \textbf{bố cục Server Component} của mỗi nhóm tuyến.
Bố cục của nhóm \code{(app)} gọi \code{getMe()} một lần rồi điều hướng theo
trạng thái: chưa đăng nhập $\rightarrow$ \code{/login}; đăng nhập nhưng chưa
khai báo hồ sơ $\rightarrow$ \code{/onboarding}; hợp lệ thì dựng khung ứng dụng.
Cách đặt cổng kiểm soát ở biên kết xuất phía máy chủ bảo đảm trang chỉ được
dựng \textit{sau khi} đã xác thực, và vì \code{getMe} được ghi nhớ
(mục~\ref{subsec:fe-data}) nên việc kiểm tra này không tốn thêm lượt gọi mạng
cho các trang con.

\begin{verbatim}
// src/app/(app)/layout.tsx — cổng kiểm soát đặt trong bố cục
const user = await getMe();
if (!user) redirect('/login');
if (!user.isOnboarded) redirect('/onboarding');
\end{verbatim}

\subsection{Thu âm phía máy khách cho luyện phát âm}
\label{subsec:fe-audio}

Câu hỏi luyện phát âm cần một bản ghi âm đúng định dạng mà vi dịch vụ chấm điểm
(mục~\ref{subsec:impl-pron}) chấp nhận. Hook \code{useWavRecorder}
(\code{learn/questions/_shared/}) \textit{không} dùng \code{MediaRecorder} ---
API này tạo tệp \code{webm/opus} bị dịch vụ chấm điểm từ chối --- mà thu PCM thô
qua \textbf{Web Audio API}: mở micro bằng \code{getUserMedia}, lấy mẫu qua
\code{AudioContext}, hạ tần số về \textbf{16~kHz đơn kênh} rồi tự đóng gói thành
tệp \textbf{WAV} 16-bit ngay trên trình duyệt~\cite{w3cwebaudio, w3cmediacapture}.
Hook còn phơi ra một mức biên độ RMS đã làm mượt để vẽ thanh đo âm lượng trực
tiếp, và tự giải phóng micro khi câu hỏi bị tháo khỏi giao diện giữa chừng. Tệp
WAV sau đó được gửi kèm trong một \code{FormData} qua Server Action tới điểm cuối
chấm phát âm.

\subsection{Giao diện khu vực quản trị}
\label{subsec:fe-admin}

Khu vực \code{(admin)} hiện thực phần biên tập nội dung mà quản trị viên cần,
bám sát các use case quản trị (Bảng~\ref{tab:fr-vocab}, \ref{tab:fr-ai}):

\begin{itemize}
  \item \textbf{Trình soạn từ vựng:} mỗi từ là một trang chi tiết
        (\code{vocabularies/[id]/}) với cây nghĩa--bản dịch--ví dụ được sửa
        \textit{tại chỗ} (inline) theo từng khối nhỏ; mỗi khối là một
        \code{ActionForm} riêng nên thao tác sửa chỉ làm tươi đúng phần liên quan.
  \item \textbf{Hàng đợi duyệt nội dung AI:} trang \code{vocabularies/review/}
        liệt kê các bản nháp do AI sinh (\code{is_approved = false}) để quản trị
        viên xem, sửa và phê duyệt trước khi xuất bản vào kho hệ thống
        (mục~\ref{subsec:impl-enrich}).
  \item \textbf{Tạo và làm giàu hàng loạt:} luồng \code{tạo nhanh} (\code{quick/})
        cho một lemma, và \textit{trình thuật sĩ nhập hàng loạt} nhiều bước
        (\code{bulk/}: chọn nguồn $\rightarrow$ xác nhận $\rightarrow$ theo dõi
        tiến độ); trang \code{jobs/} theo dõi trạng thái các công việc làm giàu
        bất đồng bộ (mục~\ref{subsec:impl-bulk}).
  \item \textbf{Quản trị chủ đề và người dùng:} các tuyến \code{admin/topics}
        và \code{admin/users} cho thao tác quản trị danh mục chủ đề và tài khoản.
\end{itemize}

Khu vực quản trị giữ \textit{chủ đề trung tính} riêng (không dùng bộ token thương
hiệu của khu vực học), phản ánh sự tách biệt giữa công cụ vận hành và trải nghiệm
người học.
...
tự giới hạn tần suất và suy giảm có kiểm soát. Bổ sung cho nhóm này, \textit{phòng
luyện nói} đưa LLM (Groq/Llama~3.3) vào một vòng hội thoại nhập vai theo lượt
chạy đồng bộ, với cơ chế chốt bản sao kịch bản theo phiên bản, hạn mức phiên/lượt
bảo vệ khoá dùng chung và báo cáo phản hồi cuối phiên sinh lại được. Ở phía
\textit{máy khách}, một
