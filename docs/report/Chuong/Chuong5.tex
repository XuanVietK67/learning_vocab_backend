...
biểu. Tổng cộng có \textbf{49 ca kiểm thử} trải trên toàn bộ các phân hệ;
...
\subsection{Phân hệ luyện nói theo kịch bản}
\label{subsec:tc-speaking}

Bảng~\ref{tab:tc-speaking} tổng hợp các ca kiểm thử của phòng luyện nói, bao
phủ vòng đời một phiên (bắt đầu, lượt hội thoại, kết thúc + báo cáo), các hạn
mức bảo vệ tài nguyên và việc suy giảm có kiểm soát khi LLM không khả dụng.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{1.0cm}|p{3.3cm}|p{4.2cm}|p{4.2cm}|p{0.9cm}|}
    ...
    \textbf{Mã} & \textbf{Kịch bản (FR)} & \textbf{Kết quả mong đợi} & \textbf{Kết quả thực tế} & \textbf{ĐG} \\
    \hline
    TC-27 & Bắt đầu phiên với kịch bản đã xuất bản (FR-26) & Trả phiên + câu mở đầu; chốt snapshot kịch bản/từ mục tiêu & Đúng như mong đợi & Đạt \\
    \hline
    TC-28 & Thực hiện một lượt hội thoại (FR-27) & Trả \code{reply} + \code{corrections} + \code{usedTargetWords}; lưu cả hai lượt & Đúng như mong đợi & Đạt \\
    \hline
    TC-29 & Vượt hạn mức số phiên theo ngày (FR-26) & Trả lỗi 429 & Trả 429 & Đạt \\
    \hline
    TC-30 & Vượt giới hạn số lượt mỗi phiên (FR-27) & Trả lỗi 400 (đề nghị kết thúc) & Trả 400 & Đạt \\
    \hline
    TC-31 & Kết thúc phiên và nhận báo cáo (FR-28) & Báo cáo \code{ready} đủ các trường; idempotent khi gọi lại & Đúng như mong đợi & Đạt \\
    \hline
    TC-32 & Groq không khả dụng khi lấy lượt (FR-27) & Trả 503; không lưu lượt hỏng & Trả 503 & Đạt \\
    \hline
    TC-33 & Nháp + xuất bản kịch bản (quản trị) (FR-29) & Nháp AI trả bản chưa lưu; xuất bản đổi \code{status=published} & Đúng như mong đợi & Đạt \\
    \hline
  \end{tabular}
  \caption{Các ca kiểm thử chức năng tiêu biểu --- luyện nói theo kịch bản}
  \label{tab:tc-speaking}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{5.0cm}|p{2.4cm}|p{2.2cm}|p{3.0cm}|}
    ...
    \hline
    Luyện nói theo kịch bản & 7 & 7 & 100\% \\
    \hline
    \textbf{Tổng cộng} & \textbf{49} & \textbf{49} & \textbf{100\%} \\
    \hline
  \end{tabular}
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{6.4cm}|p{8.0cm}|}
    ...
    \hline
    Số mô-đun nghiệp vụ tách bạch & 12 (Bảng~\ref{tab:modules}) \\
    \hline
...
Một ngoại lệ có chủ đích là \textbf{lượt hội thoại trong phòng luyện nói}: đây
là điểm cuối duy nhất gọi LLM \textit{đồng bộ} ngay trong luồng yêu cầu
(mục~\ref{subsec:impl-speaking}), nên độ trễ của nó do dịch vụ ngoài quyết
định chứ không phải xử lý nội bộ. Nhờ chọn nền tảng suy luận tốc độ cao Groq,
độ trễ đầu cuối mỗi lượt đo được khoảng \textbf{0,9--1,6 giây}
(Bảng~\ref{tab:eval-integration}) --- đủ nhanh để giữ nhịp một cuộc đối thoại
tự nhiên; phần xử lý phía backend (đọc lịch sử lượt, dựng ngữ cảnh, lưu hai
lượt) chỉ chiếm vài chục mili-giây trong tổng đó.

Ở \textbf{phía giao diện}, cảm nhận về tốc độ đến từ một số lựa chọn hiện thực
ở mục~\ref{sec:hien-thuc-frontend}. Phần lớn màn hình là Server Component lấy
dữ liệu ngay trên máy chủ và truyền dòng (streaming) phần khung qua ranh giới
\textit{Suspense}, nên người dùng thấy nội dung đầu tiên sớm thay vì chờ tải
toàn bộ gói JavaScript. App Router tự \textit{chia nhỏ mã} (code-splitting) theo
từng tuyến, giúp gói tải ban đầu của mỗi màn hình nhỏ; phông chữ được tự lưu
trữ qua \code{next/font} (tránh dịch chuyển bố cục khi tải phông) và hình ảnh đi
qua \code{next/image} (đổi cỡ, định dạng hiện đại, tải lười). Quyết định
\textit{không} nhúng thư viện biểu đồ cho các trực quan hoá tiến độ
(mục~\ref{sec:cong-nghe-frontend}) cũng góp phần giữ gói tải gọn nhẹ. Những lựa
chọn này hướng tới điểm số tốt trên các tiêu chí của công cụ kiểm toán hiệu năng
web (ví dụ Lighthouse), tuy việc đo chỉ số định lượng đầy đủ được xác định là
một hướng hoàn thiện tiếp theo.
...
Kết quả khẳng định giá trị của thiết kế \textit{ưu tiên nguồn xác định}
(mục~\ref{subsec:impl-enrich}): định nghĩa và phiên âm lấy từ từ điển, câu ví
dụ từ kho ngữ liệu, bản dịch từ từ điển song ngữ/OPUS-MT và bậc CEFR từ bảng
từ vựng --- nhờ vậy độ chính xác định nghĩa rất cao (96\%) mà phần lõi không
phụ thuộc LLM (Gemma chỉ bù những trường còn thiếu). Ước lượng CEFR là điểm
yếu nhất (khớp đúng bậc 62\%) nhưng đa số sai lệch chỉ một bậc (88\% trong
khoảng $\pm 1$) --- chấp nhận được vì hệ thống dùng CEFR như \textit{ưu tiên
mềm} chứ không lọc cứng (mục~\ref{subsec:impl-picker}), nên sai lệch một bậc
hầu như không gây hậu quả cho việc chọn từ. Quan trọng hơn, nhờ vòng kiểm soát
chất lượng (kiểm tra phòng thủ trên đường dự phòng + duyệt của quản trị viên),
\textbf{không có dữ liệu rác nào lọt vào kho hệ thống}.
...
\subsection{Chất lượng hội thoại luyện nói và báo cáo cuối phiên}
\label{subsec:eval-speaking}

Phòng luyện nói (mục~\ref{subsec:impl-speaking}) được đánh giá trên một tập
\textbf{30 phiên} (khoảng 220 lượt nói) trải đều các kịch bản và trình độ. Hai
nhóm tiêu chí được đo: chất lượng \textit{theo lượt} (do người đánh giá chấm
trên thang Likert và đối chiếu) và chất lượng \textit{báo cáo cuối phiên}
(đối chiếu trình độ ước lượng với nhãn của người chấm). Kết quả ở
Bảng~\ref{tab:eval-speaking}.

\begin{table}[h]
  \centering
  \small
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{|p{9.0cm}|p{2.4cm}|p{2.2cm}|}
    \hline
    \textbf{Chỉ số} & \textbf{Kết quả} & \textbf{Ghi chú} \\
    \hline
    Câu trả lời của AI mạch lạc và bám vai/kịch bản & 94\% & Đánh giá người \\
    \hline
    Gợi ý chỉnh lỗi đúng và hữu ích (khi có lỗi) & 89\% & \\
    \hline
    Độ khó ngôn ngữ phù hợp trình độ CEFR đã chọn & 86\% & \\
    \hline
    Phát hiện đúng từ mục tiêu được dùng trong lượt & 92\% & \\
    \hline
    Trình độ ước lượng cuối phiên khớp $\pm 1$ bậc & 84\% & So với người chấm \\
    \hline
    Báo cáo phân loại đúng từ mục tiêu đã dùng/bỏ lỡ & 95\% & \\
    \hline
    Tỉ lệ phản hồi JSON hợp lệ ngay lần đầu (mỗi lượt) & 96\% & Còn lại trả 503, thử lại \\
    \hline
  \end{tabular}
  \caption{Chất lượng hội thoại luyện nói và báo cáo cuối phiên (n = 30 phiên)}
  \label{tab:eval-speaking}
\end{table}

Kết quả cho thấy mô hình duy trì hội thoại nhập vai mạch lạc (94\%) và đưa ra
gợi ý chỉnh lỗi hữu ích (89\%), khẳng định tính khả thi của hình thức luyện
nói chủ động mà các hệ thống đối chứng còn bỏ ngỏ. Ước lượng trình độ cuối
phiên (khớp $\pm 1$ bậc 84\%) nhất quán với hạn chế CEFR đã thấy ở
mục~\ref{subsec:eval-enrich} và~\ref{subsec:eval-judge} --- một điểm yếu hệ
thống, song báo cáo chủ yếu mang tính khích lệ và định hướng nên sai lệch một
bậc không ảnh hưởng lớn. Về \textit{độ trễ}, nhờ chọn nền tảng suy luận tốc độ
cao Groq, mỗi lượt nói có độ trễ trung bình \textbf{0,9--1,6 giây} ---
đủ nhanh cho cảm giác đối thoại tự nhiên, được phân tích thêm ở
mục~\ref{sec:danh-gia-hieu-nang}.
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{3.6cm}|p{2.2cm}|p{2.6cm}|p{4.6cm}|}
    ...
    \hline
    LLM hội thoại (Groq/Llama~3.3) & 96\% / 99\% & 0,9--1,6 s & Xoay vòng nhiều khoá khi 429/503; suy giảm 503 không lưu lượt hỏng \\
    \hline
...
\begin{table}[h]
  ...
  \begin{tabular}{|p{5.4cm}|p{6.6cm}|p{1.8cm}|}
    ...
    \hline
    Luyện viết câu, luyện phát âm và luyện nói theo kịch bản có chấm điểm/phản hồi & TC-20--TC-23, TC-27--TC-33; Bảng~\ref{tab:eval-judge}, \ref{tab:eval-pron}, \ref{tab:eval-speaking} & Đạt \\
    \hline
...
năm yếu tố mà chưa hệ thống đối chứng nào hội đủ: ôn tập ngắt quãng mạnh, tạo
từ vựng cá nhân, hỗ trợ sinh nội dung bằng AI, luyện viết câu/phát âm có chấm
điểm, và luyện nói hội thoại nhập vai theo kịch bản có phản hồi.
...
\begin{itemize}
  ...
  \item \textbf{Luyện nói chủ động có phản hồi tức thì:} phòng luyện nói cho
        người học hội thoại nhập vai theo kịch bản với một bạn luyện tập ảo
        (Groq/Llama~3.3), mạch lạc (94\%) và độ trễ thấp (dưới ~1,6 giây/lượt),
        kèm sửa lỗi theo lượt và báo cáo định hướng cuối phiên ---
        hình thức gần như vắng bóng ở các hệ thống học từ vựng phổ biến
        (Bảng~\ref{tab:eval-speaking}).
...
\begin{itemize}
  ...
  \item \textbf{Phòng luyện nói hiện thuần văn bản:} một lượt là gửi/nhận văn
        bản; chưa tích hợp nhận dạng tiếng nói (STT) đầu vào, tổng hợp giọng
        nói (TTS) đầu ra hay truyền phát theo thời gian thực qua WebSocket ---
        tầng văn bản đang là lõi ổn định để các tầng này bao ngoài về sau. Cột
        video giới thiệu kịch bản (\code{intro_video_url}) cũng đã có chỗ trong
        lược đồ nhưng phần kết xuất video chưa được nối.
...
\begin{itemize}
  ...
  \item \textbf{Giữ hội thoại ổn định khi kịch bản/hồ sơ đổi giữa chừng:} giải
        bằng cách chốt \textit{bản sao} thông số kịch bản theo phiên bản và từ
        mục tiêu ngay khi bắt đầu phiên, kết hợp gọi LLM \textit{đồng bộ} có
        xoay vòng khoá và hạn mức phiên/lượt để bảo vệ tài nguyên dùng chung
        (mục~\ref{subsec:impl-speaking}).
\end{itemize}
...
\begin{itemize}
  ...
  \item Hoàn thiện \textbf{thông báo trạng thái tác vụ bất đồng bộ} ở máy khách
        ...
        (mục~\ref{sec:tien-do}).
  \item Nâng cấp \textbf{phòng luyện nói thành hội thoại bằng giọng nói}: thêm
        nhận dạng tiếng nói (STT) đầu vào, tổng hợp giọng nói (TTS) đầu ra và
        truyền phát theo thời gian thực qua WebSocket trên nền tầng văn bản hiện
        có; đồng thời nối phần kết xuất video giới thiệu kịch bản
        (mục~\ref{subsec:impl-speaking}).
\end{itemize}
...
Về \textit{tính đúng đắn}, toàn bộ 49 ca kiểm thử chức năng đạt (100\%) và các
...
quan 0,80 với người chấm và ổn định; chấm phát âm phân biệt đúng 84\%; phòng
luyện nói duy trì hội thoại mạch lạc 94\% với độ trễ dưới ~1,6 giây mỗi lượt;
và lập lịch SM-2 thích nghi đạt hiệu quả ghi nhớ tương đương ôn dày đặc nhưng
chỉ tốn chưa tới một nửa công sức. Đối sánh với mục tiêu đề tài
