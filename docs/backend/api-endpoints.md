...
| Method | Path | Auth | Purpose |
| --- | --- | --- | --- |
| POST | `/v1/me/vocabularies` | JWT | Create a personal vocabulary. Body carries one or more `senses[]`, each with its own translations, examples, and `imageUrl`. Example sentences without a `translation` are auto-translated into the default language (OPUS-MT) before saving; a supplied one is kept. Topic slugs (vocab-level) must already exist in the system catalog. Returns 409 if the caller already has a word for `(language, lemma, partOfSpeech)`. |
...
| POST | `/v1/admin/vocabularies` | JWT (admin) | Create one system vocabulary. Body carries one or more `senses[]` (each with translations, examples, and per-sense `imageUrl`) plus vocab-level topic links by slug. Example sentences without a `translation` are auto-translated into the default language (OPUS-MT) before saving; a supplied one is kept. Returns 409 if `(language, lemma, partOfSpeech)` already exists — use bulk-import for upsert semantics. |
...
| POST | `/v1/admin/vocabularies/:id/senses` | JWT (admin) | Append a new sense to a vocabulary (`senseOrder` auto-assigned to `max+1`). Body: `{ gloss?, definition?, imageUrl?, translations?, examples? }`. Example sentences without a `translation` are auto-translated into the default language (OPUS-MT). Returns 201 with the created sense and its children. |
...
| POST | `/v1/admin/vocabularies/:id/senses/:senseId/examples` | JWT (admin) | Add an example sentence to a sense. Body: `{ sentence, translation?, source? }`. When `translation` is omitted it is auto-translated into the default language (OPUS-MT). |
